...
const WeatherList = ({ weatherData }) => {
  ...
  return (
    <div className='flex flex-col'>
      <h1 className='font-[600] text-[px] text-center text-[30px]'>Weekly</h1>
      <div className='flex gap-[1%] flex-wrap p-5 overflow-auto max-h-[260px] no-scroll-bar relative'>
        {weatherData?.four_futures?.map((data) => (
          <div
            key={data.date}
            className='max-xl:basis-[32%] bg-blue-400 rounded-md max-lg:basis-[49%] max-sm:basis-[100%] basis-[24%] mb-[2%]'
          >
            <WeatherCard
              data={data}
              plain
